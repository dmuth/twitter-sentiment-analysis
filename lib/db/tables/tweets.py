...
class data():
	...
	def __init__(self, db):
		...
		schema = ("username TEXT NOT NULL, date TEXT NOT NULL, time_t INTEGER, "
			...
			+ "sentiment TEXT NOT NULL, score TEXT NOT NULL, exported TEXT NOT NULL, "
			+ "url TEXT, age INTEGER "
			)
	...
	#
	def put(self, username, date, time_t, tweet_id, tweet, profile_image, url, age):

		#
		# Got the idea for this query from https://stackoverflow.com/a/4330694/196073
		#
		query = ("INSERT OR REPLACE INTO %s (username, date, time_t, tweet_id, tweet, profile_image, url, age, sentiment, score, exported) "
			"VALUES (?, ?, ?, ?, ?, ?, ?, ?, "
			"COALESCE( (SELECT sentiment FROM tweets WHERE tweet_id = ?), ''), "
			"COALESCE( (SELECT score FROM tweets WHERE tweet_id = ?), ''), "
			"COALESCE( (SELECT exported FROM tweets WHERE tweet_id = ?), '') "
			")" % self.table)

		self.db.execute(query, (username, date, time_t, tweet_id, tweet, 
			profile_image, url, age, tweet_id, tweet_id, tweet_id))
